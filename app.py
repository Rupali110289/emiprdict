...
st.title("🏦 EMI Eligibility & EMI Amount Prediction App")

st.write("Fill the customer details below:")
...
if st.button("🔍 Predict"):
    ...
    # Classification
    status = predict_eligibility(engineered)

    # Regression
    max_emi = predict_emi_amount(engineered)

    st.subheader("✅ Results")
    st.write(f"### EMI Eligibility: {status}")
    st.write(f"### Maximum EMI You Can Pay: **₹{max_emi:,.2f}**")


st.info("✅ Models auto-download from Google Drive at startup.")
